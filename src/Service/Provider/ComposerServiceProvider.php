...
final class ComposerServiceProvider extends AbstractProvider implements ComposerServiceProviderInterface
{
     ...
     * @throws Throwable
     */
    private function addProvidersFromPackage(array $package): void
    {
        ...
        if (! is_array($providers)) {
            throw new ShouldNotHappenException(
                sprintf(
                    ...
                    implode('.', self::COMPOSER_EXTRA_GHOSTWRITER_CONTAINER_PROVIDER),
    ...
    private function decodeJsonFile(string $path): array
    {
        ...
        throw new ShouldNotHappenException(sprintf('Could not decode %s at %s', basename($path), $path));
    }
     ...
     * @return array<string, mixed>
     */
    private function getComposerJson(string $composerJsonPath): array
    {
        return $this->decodeJsonFile($composerJsonPath);
    }
     ...
     */
    private function getInstalledJson(string $installedJsonPath): array
    {
        $installedJson = $this->decodeJsonFile($installedJsonPath);
    ...
    private function packageProviderValue(array $package): mixed
    {
        ...
        foreach (self::COMPOSER_EXTRA_GHOSTWRITER_CONTAINER_PROVIDER as $key) {
    ...
    private function projectRootPath(): string
    {
        ...
        while (! is_file($this->path($currentDirectory, self::VENDOR_AUTOLOAD_SEGMENTS))) {
    ...
    private function readFileContents(string $path): string
    {
        ...
        if (false === $contents) {
            throw new ShouldNotHappenException(sprintf('Could not read %s at %s', basename($path), $path));
        }
